...
class VehicleNoGPSService {
  async getVehicleNoGPS(userID: number) {
    try {
      ...
      try {
        const data = await vehicleNoGPSModel.getAllRowsByUserID(conn, userID);
        return data;
      } catch (error) {
        throw error;
      } finally {
  ...
  async getVehicleNoGPSbyID(data: any) {
    try {
      const { conn } = await getConnection();
      try {
        const vehicle = await vehicleNoGPSModel.getVehicleNoGPSbyID(conn, data);
        return vehicle;
      } catch (error) {
        throw error;
      } finally {
  ...
  async deleteVehicleNoGPS(vehicleID :number, userID :number) {
    try {
        const { conn } = await getConnection();
        try {
          const result = vehicleNoGPSModel.deleteVehicleNoGPS(conn, userID,vehicleID);
          return result;
        } catch (error) {
          throw error;
        } finally {
